# -*- coding: utf-8 -*-
"""
Created on Fri Nov 22 10:34:40 2019
...
syst = chrono.ChSystemSMC()
contact_method= chrono.ChMaterialSurface.SMC
syst.Set_G_acc(chrono.ChVectorD(0.,0.,0.))
...
#align the shape to the center of axis system
...
mvisualisebeamA=fea.ChVisualizationFEAmesh(mesh)
mvisualisebeamA.SetFEMdataType(fea.ChVisualizationFEAmesh.E_PLOT_ELEM_BEAM_MZ)
mvisualisebeamA.SetColorscaleMinMax(-0.4,0.4)
mvisualisebeamA.SetSmoothFaces(True)
mvisualisebeamA.SetWireframe(False)
mesh.AddAsset(mvisualisebeamA)
...
mAp = chronoirr.ChIrrApp(syst,'Cloth Simulation',
                         chronoirr.dimension2du(800,600))

mAp.AddTypicalSky()
mAp.AddTypicalCamera(chronoirr.vector3df(0.3,0.,0.3))
mAp.AddTypicalLights()
mAp.SetShowInfos(False)
...
# Run simulation
#
# Change the solver form the default SOR to MKL Pardiso, more precise for FEA.

msolver = mkl.ChSolverMKLcsm()
syst.SetSolver(msolver)
mAp.SetTimestep(0.001)

while mAp.GetDevice().run():
    mAp.BeginScene()
    mAp.DrawAll()
    mAp.DoStep()
    mAp.EndScene()
